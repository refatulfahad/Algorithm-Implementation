...
const int N=3e4+5;
int ar[N],n,par[N][18],sz[N],level[N],num,chain_id[N],ar_index[N];
vector<int>vertex[N],chain[N],bit[N];
...
void update(int sz,int pos,int id,int val) {
    while(pos<=sz) {
        bit[id][pos]+=val;
        pos+=(pos&-pos);
    }
...
void initial() {
    for(int i=1; i<=num; i++) {
        ...
        for(int j=1; j<=(int)chain[i].size(); j++) {
            update(chain[i].size(),j,i,ar[chain[i][j-1]]);
        }
    }
}
...
int hld_query(int node1,int node2) {
    ...
    while(1) {
        ...
        if(chain_id[node1]==chain_id[lca]) {
            ans+=(query(tem,ar_index[node1])-query(tem,ar_index[lca]-1));
            break;
        }
        ans+=query(tem,ar_index[node1]);
        node1=par[chain[tem][0]][0];
    }
    ...
    while(1) {
        ...
        if(chain_id[node2]==chain_id[lca]) {
            ans+=(query(tem,ar_index[node2])-query(chain_id[lca],ar_index[lca]));
            break;
        }
        ans+=query(tem,ar_index[node2]);
        node2=par[chain[tem][0]][0];
    }
    return ans;
}
int main() {
    ...
    for(int cs=1; cs<=test; cs++) {
        ...
        while(q--) {
            ...
            if(ck==0) {
                ...
                scanf("%d %d",&node,&val);
                node++;
                update(chain[chain_id[node]].size(),ar_index[node],chain_id[node],val-ar[node]);
                ar[node]=val;
            }
        }
        for(int i=1; i<=n; i++) {
            vertex[i].clear(),chain[i].clear(),bit[i].clear();
            for(int j=1; j<=15; j++)par[i][j]=0;
        }
    }
    return 0;
}
